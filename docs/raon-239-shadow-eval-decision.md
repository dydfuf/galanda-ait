...
현재 저장소에는 실제 AI Gateway credential과 실행 환경이 없으므로 모델 품질·latency·cost를 측정한 것처럼 기록하지 않는다. Shadow 실행 경계와 재현 가능한 비교 runner까지만 활성화하고, 두 후보의 실제 report가 확보될 때까지 active mode를 보류한다.
...
두 실제 후보 report에서 위 미측정 항목이 채워지고 gate가 모두 통과하기 전에는 RAON-240 active ranking을 시작하지 않는다.
