    ...
    return (
      <PageBody>
        <PageState
          ...
          description={toUserMessage(error, "요청한 정보를 찾을 수 없습니다.")}
          actionText="다시 시도"
      ...
      {(itineraryState.itinerary.currentRevision > 1 ||
        ...
        >
          <div className="min-w-0 rounded-2xl border border-border bg-surface-content p-4 shadow-xs [overflow-wrap:anywhere]">
            ...
            {conflictNotice ? (
              <p
                className="mt-2 text-sm leading-relaxed text-warning [overflow-wrap:anywhere]"
                role="alert"
              >
                {conflictNotice}
              </p>
              ...
              acknowledgeMutation.isError && (
                <p
                  className="mt-2 text-sm leading-relaxed text-destructive [overflow-wrap:anywhere]"
                  role="alert"
                >
                  {toUserMessage(
      ...
      >
        <DrawerContent>
          <DrawerHeader>
            ...
            <DrawerDescription className="min-w-0 text-left text-base leading-relaxed [overflow-wrap:anywhere]">
          ...
          <div className="min-h-0 flex-1 overflow-y-auto">
            <MobileList aria-label="확인 필요 예약 목록" className="pb-2">
              {viewModel.needCheckItems.map((item) => (
                ...
                >
                  <ItemTitle className="text-base leading-snug [overflow-wrap:anywhere]">
                    {item.message}
                  </ItemTitle>
          ...
          <DrawerFooter>
            <Button
              type="button"
              size="xl"
              onClick={() => setIsNeedCheckSheetOpen(false)}
            >
              닫기
            </Button>
          </DrawerFooter>
      ...
      >
        <DrawerContent>
          <DrawerHeader>
            ...
            <DrawerDescription className="min-w-0 text-left text-base leading-relaxed [overflow-wrap:anywhere]">
              {itineraryState.status === "CONFIRMED" &&
              itineraryState.itinerary.changes?.length
          ...
          <div className="min-h-0 flex-1 overflow-y-auto">
            ...
            itineraryState.itinerary.changes?.length ? (
              <MobileList aria-label="변경된 일정 항목">
                {itineraryState.itinerary.changes.map(
                  ({ itemId, before, after }) => {
                    const changedFields: ReadonlyArray<{
                      label: string;
                      before: string;
                      after: string;
                    }> = (() => {
                      if (before.type === "STAY" && after.type === "STAY") {
                        ...
                        if (
                          before.accommodation.hotelName !==
                          after.accommodation.hotelName
                        ) {
                          fields.push({
                            label: "숙소",
                            before:
                              before.accommodation.hotelName || "(호텔명 없음)",
                            after:
                              after.accommodation.hotelName || "(호텔명 없음)",
                          });
                        }
                        if (
                          before.accommodation.city !== after.accommodation.city
                        ) {
                          fields.push({
                            label: "도시",
                            before: before.accommodation.city,
                            after: after.accommodation.city,
                          });
                        }
                        ...
                        if ((before.memo ?? "") !== (after.memo ?? "")) {
                          fields.push({
                            label: "메모",
                            before: before.memo?.trim() || "(메모 없음)",
                            after: after.memo?.trim() || "(메모 없음)",
                          });
                        }
                        if (
                          before.accommodation.bookingStatus !==
                          after.accommodation.bookingStatus
                        ) {
                          fields.push({
                            label: "예약 상태",
                            before: before.accommodation.bookingStatus,
                            after: after.accommodation.bookingStatus,
                          });
                        }
                        if (fields.length === 0) {
                          fields.push({
                            label: "숙소",
                            before: before.accommodation.hotelName,
                            after: after.accommodation.hotelName,
                          });
                        }
                        return fields;
                      }
                      if (
                        ...
                        if (
                          before.transport.fromCity !== after.transport.fromCity
                        ) {
                          fields.push({
                            label: "출발지",
                            before: before.transport.fromCity,
                            after: after.transport.fromCity,
                          });
                        }
                        if (
                          before.transport.toCity !== after.transport.toCity
                        ) {
                          fields.push({
                            label: "도착지",
                            before: before.transport.toCity,
                            after: after.transport.toCity,
                          });
                        }
                        if (before.transport.mode !== after.transport.mode) {
                          fields.push({
                            label: "수단",
                            before: before.transport.mode || "(수단 없음)",
                            after: after.transport.mode || "(수단 없음)",
                          });
                        }
                        if (
                          before.transport.hasTransfer !==
                          after.transport.hasTransfer
                        ) {
                          fields.push({
                            label: "환승",
                            before: before.transport.hasTransfer
                              ? "환승 필요"
                              : "직통",
                            after: after.transport.hasTransfer
                              ? "환승 필요"
                              : "직통",
                          });
                        }
                        ...
                        if (fields.length === 0) {
                          fields.push({
                            label: "이동",
                            before: `${before.transport.fromCity} → ${before.transport.toCity}`,
                            after: `${after.transport.fromCity} → ${after.transport.toCity}`,
                          });
                        }
                        return fields;
                      }
                    ...
                    const title =
                      after.type === "STAY"
                        ? after.accommodation.hotelName
                        : `${after.transport.fromCity} → ${after.transport.toCity}`;
                    return (
                      <MobileListItem key={itemId}>
                        ...
                        <div className="flex flex-col gap-1.5 pt-1.5">
                          {changedFields.map((field) => (
                            <div
                              key={field.label}
                              className="grid min-w-0 grid-cols-[52px_minmax(0,1fr)] gap-2"
                            >
                              <span className="text-xs font-bold text-muted-foreground">
                                {field.label}
                              </span>
                              <span className="min-w-0 text-[13px] break-words">
                                <span className="text-secondary-foreground line-through decoration-muted-foreground/50">
                                  {field.before}
                                </span>
              ...
              <p className="px-(--app-inline-padding) py-4 text-[13px] text-muted-foreground">
                변경된 항목이 없거나 불러오는 중이에요. 새로고침 후 다시
                확인해주세요.
              </p>
            )}
              ...
              <MobileList aria-label="일정 상세 정보" className="pb-2">
                <MobileListItem
                  trailing={
                    <Badge
                      variant={
                        selectedItem.type === "STAY" ? "info" : "neutral"
                      }
                    >
                ...
                <MobileListItem
                  trailing={
                    <Badge variant={selectedItem.statusTone}>
                ...
                <MobileListItem
                  trailing={
                    <span className="min-w-0 text-right text-base font-bold leading-relaxed text-foreground [overflow-wrap:anywhere]">
                ...
                <MobileListItem
                  trailing={
                    <span className="min-w-0 text-right text-sm leading-relaxed text-muted-foreground [overflow-wrap:anywhere]">
                ...
                {selectedItem.memo && (
                  <MobileListItem>
                    ...
                    <ItemDescription className="whitespace-pre-wrap text-base leading-relaxed [overflow-wrap:anywhere]">
                      {selectedItem.memo}
                    </ItemDescription>
                  </MobileListItem>
                )}
