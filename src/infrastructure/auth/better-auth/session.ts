...
import type { ParticipantIdentity } from "./participant-identity.ts";
import {
...
export const normalizeBetterAuthSession = (
  ...
  identity: ParticipantIdentity = {
    participantId: ParticipantIdSchema.make(session.user.id),
    participantIds: [ParticipantIdSchema.make(session.user.id)],
  }
): UserSession => ({
  ...identity,
  accountType: session.user.isAnonymous ? "GUEST" : "REGISTERED",
