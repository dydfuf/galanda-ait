  ...
  Effect.gen(function* () {
    ...
    return {
      ...
      updateRoom: (
        ...
        Effect.tryPromise({
          try: async () => {
            const { data, error } = await client.rpc("update_trip_room", {
              room_id: roomId,
              room_data: params,
      ...
      createPlan: (roomId: TripId, plan: TripPlan, expectedRevision: Revision) =>
        Effect.tryPromise({
          try: async () => {
            const { data, error } = await client.rpc("create_trip_plan", {
              room_id: roomId,
              plan_data: plan,
              expected_revision: expectedRevision,
            });
      ...
      updatePlan: (roomId: TripId, plan: TripPlan, expectedRevision: Revision) =>
        Effect.tryPromise({
          try: async () => {
            const { data, error } = await client.rpc("update_trip_plan", {
              room_id: roomId,
              plan_data: plan,
              expected_revision: expectedRevision,
            });
      ...
      deletePlan: (roomId: TripId, planId: PlanId, expectedRevision: Revision) =>
        Effect.tryPromise({
          try: async () => {
            const { data, error } = await client.rpc("delete_trip_plan", {
              room_id: roomId,
              plan_id: planId,
              expected_revision: expectedRevision,
            });
      ...
      confirmPlan: (roomId: TripId, planId: PlanId, expectedRevision: Revision) =>
        Effect.tryPromise({
          try: async () => {
            const { data, error } = await client.rpc("confirm_trip_plan", {
              room_id: roomId,
              plan_id: planId,
              expected_revision: expectedRevision,
            });
      ...
      setPlanOpinion: (
        ...
        Effect.tryPromise({
          try: async () => {
            const { data, error } = await client.rpc("set_plan_opinion", {
              room_id: roomId,
              plan_id: planId,
      ...
      joinRoom: (roomId: TripId, member: TripMember) =>
        Effect.tryPromise({
          try: async () => {
            const { data, error } = await client.rpc("join_trip_room", {
              room_id: roomId,
              member_data: member,
            });
            if (error) throw error;
